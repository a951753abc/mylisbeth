...
- **回收商店改善** — 素材依星級定價（★1:x1, ★2:x3, ★3:x6），武器依稀有度定價（普通:x1 ~ 傳說:x50）
- **8 個新成就** — NPC 任務、佈告板交易、累計收入里程碑
- **6 個新稱號** — 委託者、司令官、護衛隊長、佈告板常客、大商人、財閥（各含強弱效果權衡）
...
│       │   ├── BattleLog.jsx        # 戰鬥日誌
│       │   ├── LeaderboardPanel.jsx  # 排行榜（6 分類 + 決鬥 + 墓碑紀錄）
│       │   ├── PlayerList.jsx       # 玩家名冊（已由排行榜取代）
