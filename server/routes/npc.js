...
router.post("/hire", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    logAction(req.user.discordId, req.gameUser?.name, "npc:hire", { npcId });
    res.json(result);
  } catch (err) {
...
router.post("/fire", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    if (result.error) return res.status(400).json(result);
    logAction(req.user.discordId, req.gameUser?.name, "npc:fire", { npcId });
    res.json(result);
  } catch (err) {
...
router.post("/heal", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    if (result.error) return res.status(400).json(result);
    logAction(req.user.discordId, req.gameUser?.name, "npc:heal", { npcId, healType });
    res.json(result);
  } catch (err) {
...
router.post("/mission/start", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    if (result.error) return res.status(400).json(result);
    logAction(req.user.discordId, req.gameUser?.name, "npc:mission", { npcId, missionType });
    res.json(result);
  } catch (err) {
