...
router.post("/list-item", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    logAction(req.user.discordId, req.gameUser?.name, "market:list_item", { itemIndex, quantity, pricePerUnit });
    res.json(result);
  } catch (err) {
...
router.post("/list-weapon", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    if (result.error) return res.status(400).json(result);
    logAction(req.user.discordId, req.gameUser?.name, "market:list_weapon", { weaponIndex, totalPrice });
    res.json(result);
  } catch (err) {
...
router.post("/buy", ensureAuth, ensureNotPaused, async (req, res) => {
  try {
    ...
    if (result.error) return res.status(400).json(result);
    logAction(req.user.discordId, req.gameUser?.name, "market:buy", { listingId });
    res.json(result);
  } catch (err) {
