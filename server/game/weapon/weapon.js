...
module.exports.createWeapon = async function (cmd, user) {
    ...
    const query = { forge1: user.itemStock[cmd[2]].itemId, forge2: user.itemStock[cmd[3]].itemId };
    ...
    if (cmd[2] === cmd[3]) {
        const per = 20 + ((user.itemStock[cmd[2]].itemLevel + user.itemStock[cmd[3]].itemLevel) * 5);
        if (roll.d100Check(per)) {
            const perName = weaponPer[parseInt(user.itemStock[cmd[2]].itemId, 10) - 1];
            weapon.text += "強化成功！\n";
            weapon.text += perName;
        ...
        if (roll.d100Check(per)) {
            const perName = weaponPer[parseInt(user.itemStock[cmd[3]].itemId, 10) - 1];
            weapon.text += "強化成功！\n";
            weapon.text += perName;
            weapon.text += " 提升" + forceLevel + "點。 \n";
            weapon[perName] += forceLevel;
