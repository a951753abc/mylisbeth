...
export default function Game({ user, onLogout }) {
  ...
  if (!gameUser) {
    return <CreateCharacter user={user} onCreate={handleCreate} onLogout={onLogout} />;
  }
    ...
    <div>
      <div className="header">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <button className="btn-danger" onClick={onLogout} style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem' }}>
            登出
          </button>
        </div>
      </div>
...
function CreateCharacter({ user, onCreate, onLogout }) {
  ...
  return (
    <div className="login-page">
      ...
      <button onClick={onLogout} style={{ background: 'transparent', color: 'var(--text-secondary)', border: 'none', marginTop: '1rem' }}>
        登出
      </button>
    </div>
  );
